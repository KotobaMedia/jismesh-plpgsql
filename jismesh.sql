...
DROP TABLE IF EXISTS japan_lv1_meshes;
CREATE TABLE japan_lv1_meshes (
    meshcode bigint PRIMARY KEY
);
INSERT INTO japan_lv1_meshes (meshcode) VALUES
...
CREATE INDEX idx_japan_lv1_meshes_meshcode ON japan_lv1_meshes(meshcode);
...
BEGIN
    ...
    lat := ab * get_unit_lat('Lv1');
    lon := cd * get_unit_lon('Lv1') + 100.0;

    -- Level-specific adjustments and unit determination
    -- Uses the level determined by the to_meshlevel function
    CASE level
        WHEN 'Lv1' THEN
            unit_lat := get_unit_lat('Lv1');
            unit_lon := get_unit_lon('Lv1');
        WHEN 'X40' THEN
            -- Check standard: Rust uses e/3 == 1, e%2 == 0. Assuming e=1,2,3,4
            -- e=3,4 -> lat += UNIT_LAT_40000
            -- e=2,4 -> lon += UNIT_LON_40000
            IF e >= 3 THEN lat := lat + get_unit_lat('X40'); END IF;
            IF e % 2 = 0 THEN lon := lon + get_unit_lon('X40'); END IF;
            unit_lat := get_unit_lat('X40');
            unit_lon := get_unit_lon('X40');
        WHEN 'X20' THEN
            -- Base X40 adjustment
            IF e >= 3 THEN lat := lat + get_unit_lat('X40'); END IF;
            IF e % 2 = 0 THEN lon := lon + get_unit_lon('X40'); END IF;
            ...
            IF f >= 3 THEN lat := lat + get_unit_lat('X20'); END IF;
            IF f % 2 = 0 THEN lon := lon + get_unit_lon('X20'); END IF;
            unit_lat := get_unit_lat('X20');
            unit_lon := get_unit_lon('X20');
        WHEN 'X16' THEN
             -- Rust uses e/2, f/2. Assuming e,f = 0-4? Check standard.
             -- Assuming integer division intended.
             lat := lat + floor(e / 2.0)::FLOAT * get_unit_lat('X16');
             lon := lon + floor(f / 2.0)::FLOAT * get_unit_lon('X16');
             unit_lat := get_unit_lat('X16');
             unit_lon := get_unit_lon('X16');
        WHEN 'X8' THEN
             -- Rust uses e, f directly. Assuming e,f = 0-9? Check standard.
             lat := lat + e * get_unit_lat('X8');
             lon := lon + f * get_unit_lon('X8');
             unit_lat := get_unit_lat('X8');
             unit_lon := get_unit_lon('X8');
        WHEN 'X4' THEN
             -- Base X8 adjustment
             lat := lat + e * get_unit_lat('X8');
             lon := lon + f * get_unit_lon('X8');
             ...
             IF h >= 3 THEN lat := lat + get_unit_lat('X4'); END IF;
             IF h % 2 = 0 THEN lon := lon + get_unit_lon('X4'); END IF;
             unit_lat := get_unit_lat('X4');
             unit_lon := get_unit_lon('X4');
        WHEN 'Lv2' THEN
            lat := lat + e * get_unit_lat('Lv2');
            lon := lon + f * get_unit_lon('Lv2');
            unit_lat := get_unit_lat('Lv2');
            unit_lon := get_unit_lon('Lv2');
        WHEN 'X5' THEN
            -- Base Lv2 adjustment
            lat := lat + e * get_unit_lat('Lv2');
            lon := lon + f * get_unit_lon('Lv2');
            ...
            IF g >= 3 THEN lat := lat + get_unit_lat('X5'); END IF;
            IF g % 2 = 0 THEN lon := lon + get_unit_lon('X5'); END IF;
            unit_lat := get_unit_lat('X5');
            unit_lon := get_unit_lon('X5');
        WHEN 'X2_5' THEN
            -- Base Lv2 adjustment
            lat := lat + e * get_unit_lat('Lv2');
            lon := lon + f * get_unit_lon('Lv2');
            ...
            IF h >= 3 THEN lat := lat + get_unit_lat('X2_5'); END IF;
            IF h % 2 = 0 THEN lon := lon + get_unit_lon('X2_5'); END IF;
            unit_lat := get_unit_lat('X2_5');
            unit_lon := get_unit_lon('X2_5');
        WHEN 'X2' THEN
            -- Base Lv2 adjustment
            lat := lat + e * get_unit_lat('Lv2');
            lon := lon + f * get_unit_lon('Lv2');
            -- X2 adjustment (g, h = 0-4?) Check standard. Rust uses g/2, h/2.
            lat := lat + floor(g / 2.0)::FLOAT * get_unit_lat('X2');
            lon := lon + floor(h / 2.0)::FLOAT * get_unit_lon('X2');
            unit_lat := get_unit_lat('X2');
            unit_lon := get_unit_lon('X2');
        WHEN 'Lv3' THEN
            lat := lat + e * get_unit_lat('Lv2'); -- Base Lv2
            lon := lon + f * get_unit_lon('Lv2');
            lat := lat + g * get_unit_lat('Lv3'); -- Lv3 part
            lon := lon + h * get_unit_lon('Lv3');
            unit_lat := get_unit_lat('Lv3');
            unit_lon := get_unit_lon('Lv3');
        WHEN 'Lv4' THEN
            lat := lat + e * get_unit_lat('Lv2'); lat := lat + g * get_unit_lat('Lv3'); -- Base Lv3
            lon := lon + f * get_unit_lon('Lv2'); lon := lon + h * get_unit_lon('Lv3');
            -- Lv4 adjustment (i=1,2,3,4)
            -- i=3,4 -> lat += UNIT_LAT_LV4
            -- i=2,4 -> lon += UNIT_LON_LV4
            IF i >= 3 THEN lat := lat + get_unit_lat('Lv4'); END IF;
            IF i % 2 = 0 THEN lon := lon + get_unit_lon('Lv4'); END IF;
            unit_lat := get_unit_lat('Lv4');
            unit_lon := get_unit_lon('Lv4');
        WHEN 'Lv5' THEN
            lat := lat + e * get_unit_lat('Lv2'); lat := lat + g * get_unit_lat('Lv3'); -- Base Lv3
            lon := lon + f * get_unit_lon('Lv2'); lon := lon + h * get_unit_lon('Lv3');
            IF i >= 3 THEN lat := lat + get_unit_lat('Lv4'); END IF; -- Base Lv4 part
            IF i % 2 = 0 THEN lon := lon + get_unit_lon('Lv4'); END IF;
            -- Lv5 adjustment (j=1,2,3,4)
            -- j=3,4 -> lat += UNIT_LAT_LV5
            -- j=2,4 -> lon += UNIT_LON_LV5
            IF j >= 3 THEN lat := lat + get_unit_lat('Lv5'); END IF;
            IF j % 2 = 0 THEN lon := lon + get_unit_lon('Lv5'); END IF;
            unit_lat := get_unit_lat('Lv5');
            unit_lon := get_unit_lon('Lv5');
        WHEN 'Lv6' THEN
            lat := lat + e * get_unit_lat('Lv2'); lat := lat + g * get_unit_lat('Lv3'); -- Base Lv3
            lon := lon + f * get_unit_lon('Lv2'); lon := lon + h * get_unit_lon('Lv3');
            IF i >= 3 THEN lat := lat + get_unit_lat('Lv4'); END IF; -- Base Lv4 part
            IF i % 2 = 0 THEN lon := lon + get_unit_lon('Lv4'); END IF;
            IF j >= 3 THEN lat := lat + get_unit_lat('Lv5'); END IF; -- Base Lv5 part
            IF j % 2 = 0 THEN lon := lon + get_unit_lon('Lv5'); END IF;
            -- Lv6 adjustment (k=1,2,3,4)
            -- k=3,4 -> lat += UNIT_LAT_LV6
            -- k=2,4 -> lon += UNIT_LON_LV6
            IF k >= 3 THEN lat := lat + get_unit_lat('Lv6'); END IF;
            IF k % 2 = 0 THEN lon := lon + get_unit_lon('Lv6'); END IF;
            unit_lat := get_unit_lat('Lv6');
            unit_lon := get_unit_lon('Lv6');
...
BEGIN
    ...
    CASE level
        WHEN 'Lv1' THEN
            ...
            ab := floor(rem_lat_lv0 / get_unit_lat('Lv1'));
            cd := floor(rem_lon_lv0 / get_unit_lon('Lv1'));
            RETURN ab * 100 + cd;
        ...
        WHEN 'X40' THEN
            ...
            cd := floor(rem_lon_lv0 / get_unit_lon('Lv1'));
            base_lv1 := ab * 100 + cd;
            rem_lat_lv1 := fmod(lat, get_unit_lat('Lv1'));
            rem_lon_lv1 := fmod(lon, get_unit_lon('Lv1'));
            e := floor(rem_lat_lv1 / get_unit_lat('X40')) * 2 + floor(rem_lon_lv1 / get_unit_lon('X40')) + 1;
        ...
        WHEN 'X20' THEN
            ...
            rem_lat_40000 := fmod(rem_lat_lv1, get_unit_lat('X40'));
            rem_lon_40000 := fmod(rem_lon_lv1, get_unit_lon('X40'));
            f := floor(rem_lat_40000 / get_unit_lat('X20')) * 2 + floor(rem_lon_40000 / get_unit_lon('X20')) + 1;
            g := 5;
        ...
        WHEN 'X16' THEN
            ...
            rem_lat_lv1 := fmod(lat, get_unit_lat('Lv1'));
            rem_lon_lv1 := fmod(lon, get_unit_lon('Lv1'));
            e := floor(rem_lat_lv1 / get_unit_lat('X16')) * 2; -- Index 0..4 -> 0,2,4,6,8
            f := floor(rem_lon_lv1 / get_unit_lon('X16')) * 2; -- Index 0..4 -> 0,2,4,6,8
            g := 7;
        ...
        WHEN 'Lv2' THEN
            ...
            rem_lat_lv1 := fmod(lat, get_unit_lat('Lv1'));
            rem_lon_lv1 := fmod(lon, get_unit_lon('Lv1'));
            e := floor(rem_lat_lv1 / get_unit_lat('Lv2'));
            f := floor(rem_lon_lv1 / get_unit_lon('Lv2'));
        ...
        WHEN 'X8' THEN
            ...
            rem_lat_lv1 := fmod(lat, get_unit_lat('Lv1'));
            rem_lon_lv1 := fmod(lon, get_unit_lon('Lv1'));
            e := floor(rem_lat_lv1 / get_unit_lat('X8')); -- Index 0..4
            f := floor(rem_lon_lv1 / get_unit_lon('X8')); -- Index 0..4
            g := 6;
            RETURN base_lv1 * 1000 + e * 100 + f * 10 + g;
        ...
        WHEN 'X5' THEN
            ...
            rem_lat_lv2 := fmod(rem_lat_lv1, get_unit_lat('Lv2'));
            rem_lon_lv2 := fmod(rem_lon_lv1, get_unit_lon('Lv2'));
            g := floor(rem_lat_lv2 / get_unit_lat('X5')) * 2 + floor(rem_lon_lv2 / get_unit_lon('X5')) + 1;
        ...
        WHEN 'X4' THEN
            ...
            rem_lat_lv1 := fmod(lat, get_unit_lat('Lv1'));
            rem_lon_lv1 := fmod(lon, get_unit_lon('Lv1'));
            e := floor(rem_lat_lv1 / get_unit_lat('X8'));
            f := floor(rem_lon_lv1 / get_unit_lon('X8'));
            g := 6;
            base_8000 := base_lv1 * 1000 + e * 100 + f * 10 + g;
            rem_lat_8000 := fmod(rem_lat_lv1, get_unit_lat('X8'));
            rem_lon_8000 := fmod(rem_lon_lv1, get_unit_lon('X8'));
            h := floor(rem_lat_8000 / get_unit_lat('X4')) * 2 + floor(rem_lon_8000 / get_unit_lon('X4')) + 1;
            i := 7;
        ...
        WHEN 'X2_5' THEN
            ...
            rem_lat_5000 := fmod(rem_lat_lv2, get_unit_lat('X5'));
            rem_lon_5000 := fmod(rem_lon_lv2, get_unit_lon('X5'));
            h := floor(rem_lat_5000 / get_unit_lat('X2_5')) * 2 + floor(rem_lon_5000 / get_unit_lon('X2_5')) + 1;
            i := 6;
        ...
        WHEN 'X2' THEN
            ...
            rem_lat_lv2 := fmod(rem_lat_lv1, get_unit_lat('Lv2'));
            rem_lon_lv2 := fmod(rem_lon_lv1, get_unit_lon('Lv2'));
            g := floor(rem_lat_lv2 / get_unit_lat('X2')) * 2; -- Index 0..4 -> 0,2,4,6,8
            h := floor(rem_lon_lv2 / get_unit_lon('X2')) * 2; -- Index 0..4 -> 0,2,4,6,8
            i := 5;
        ...
        WHEN 'Lv3' THEN
            ...
            rem_lat_lv2 := fmod(rem_lat_lv1, get_unit_lat('Lv2'));
            rem_lon_lv2 := fmod(rem_lon_lv1, get_unit_lon('Lv2'));
            g := floor(rem_lat_lv2 / get_unit_lat('Lv3'));
            h := floor(rem_lon_lv2 / get_unit_lon('Lv3'));
        ...
        WHEN 'Lv4' THEN
            ...
            rem_lat_lv3 := fmod(rem_lat_lv2, get_unit_lat('Lv3'));
            rem_lon_lv3 := fmod(rem_lon_lv2, get_unit_lon('Lv3'));
            i := floor(rem_lat_lv3 / get_unit_lat('Lv4')) * 2 + floor(rem_lon_lv3 / get_unit_lon('Lv4')) + 1;
        ...
        WHEN 'Lv5' THEN
            ...
            rem_lat_lv4 := fmod(rem_lat_lv3, get_unit_lat('Lv4'));
            rem_lon_lv4 := fmod(rem_lon_lv3, get_unit_lon('Lv4'));
            j := floor(rem_lat_lv4 / get_unit_lat('Lv5')) * 2 + floor(rem_lon_lv4 / get_unit_lon('Lv5')) + 1;
        ...
        WHEN 'Lv6' THEN
            ...
            rem_lat_lv5 := fmod(rem_lat_lv4, get_unit_lat('Lv5'));
            rem_lon_lv5 := fmod(rem_lon_lv4, get_unit_lon('Lv5'));
            k := floor(rem_lat_lv5 / get_unit_lat('Lv6')) * 2 + floor(rem_lon_lv5 / get_unit_lon('Lv6')) + 1;
...
BEGIN
    ...
    unit_lat := get_unit_lat(level);
    unit_lon := get_unit_lon(level);
    ...
    WHILE y < max_lat LOOP
        ...
        WHILE x < max_lon LOOP
            ...
            IF NOT EXISTS (SELECT 1 FROM japan_lv1_meshes j WHERE j.meshcode = base_lv1) THEN
                RAISE DEBUG 'Skipping meshcode %: not in japan_lv1_meshes', base_lv1;
                x := x + unit_lon;
                CONTINUE;
            END IF;

            geom := to_meshpoly_geom(meshcode);
...
-- Drop the old functions first if they exist
DROP FUNCTION IF EXISTS get_unit_lat(mesh_level);
DROP FUNCTION IF EXISTS get_unit_lon(mesh_level);
DROP TABLE IF EXISTS mesh_level_units;

-- 1. Create the lookup table
CREATE TABLE mesh_level_units (
    level mesh_level PRIMARY KEY,
...
BEGIN
    INSERT INTO mesh_level_units (level, unit_lat, unit_lon) VALUES
...
CREATE OR REPLACE FUNCTION get_unit_lat(p_level mesh_level)
RETURNS double precision AS $$
    SELECT m.unit_lat
    FROM mesh_level_units m
    WHERE m.level = p_level
    LIMIT 1; -- Good practice, though PRIMARY KEY ensures max 1 row
$$ LANGUAGE SQL IMMUTABLE STRICT;

CREATE OR REPLACE FUNCTION get_unit_lon(p_level mesh_level)
RETURNS double precision AS $$
    SELECT m.unit_lon
    FROM mesh_level_units m
    WHERE m.level = p_level
